...
const pathLabels: Record<string, string> = {
  ...
  '/mypage/eco-report': '環境配慮商品購入レポート',
  '/mypage/special-order': '掲載外商品取寄せ',
  '/mypage/special-order/new': '取寄せ依頼',
  '/mypage/special-order/templates': '取寄せ定番一覧',
  '/mypage/special-order/history': '過去の取寄せ一覧',
  '/favorites': 'お気に入り',
  '/quick-order': 'クイックオーダー',
  '/special-order': '掲載外商品お取り寄せ',
  '/compare': '商品比較BOX',
  '/catalog': 'デジタルカタログ',
  '/magazine': 'スマートワークス・マガジン',
  '/tv': 'JTX TV',
  '/quotations': '見積り依頼検索',
  '/quotations/new': '新規作成',
  '/help': 'ヘルプ',
  '/help/guide': 'ご利用ガイド',
  '/help/faq': 'よくある質問',
  '/help/delivery': 'お届け日について',
  '/help/cancel': 'キャンセルについて',
  '/help/return': '返品について',
  '/help/contact': 'お問い合わせ窓口について',
  '/contact': 'お問い合わせ',
};
        ...
        <nav className="ec-breadcrumb__nav flex items-center space-x-2 text-sm text-gray-600">
          {breadcrumbItems.map((item, index) => (
            <div key={index} className="ec-breadcrumb__item flex items-center space-x-2">
              ...
              {item.href ? (
                <Link href={item.href} className="ec-breadcrumb__link text-black hover:text-blue-800 transition-colors">
                  {item.label}
                </Link>
              ) : (
